...
# #get forecaster
# forecaster = Forecaster(id = get_learning_method(model))
# #predict 
# ypred = forecast(forecaster = forecaster, models = model, newx = x, type = get_prediction_type(object = forecaster, type.measure = "class_report"), newoffset = offset)


if(has.olddata){
  
  sets = c("train", "test", "full")
  section.titles = c("Training data", "Testing data", "Whole data")
  
  for(i in 1L:3L){
    
    if(tabset){cat(paste('###', section.titles[i], "{.tabset}\n"))}else{cat(paste('###', section.titles[i], "\n"))}
